    ...
    try:
            ...
            for job in jobs:
                if job == "test":
                    _banner("Job: test")
                    py = _python()
                    _step("Install", [py, "-m", "pip", "install", "-e", ".[dev]"], cwd=root, env=env)
                    _step("Ruff", [py, "-m", "ruff", "check", "src", "tests"], cwd=root, env=env)
                    _step("Compile", [py, "-m", "compileall", "-q", "src"], cwd=root, env=env)
                    _step("Pytest", [py, "-m", "pytest", "tests/", "-q"], cwd=root, env=env)
                else:
